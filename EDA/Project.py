...
class EcommerceAnalyticsApp:
    ...
    def setup_ui(self):
        ...
        side = ttk.Frame(main, relief=tk.RAISED, borderwidth=2)
        side.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

        self._make_section(side, "Data Import", [("Upload Excel/CSV", self.upload_file)])
        self.file_label = ttk.Label(side, text="No file selected", font=('Arial', 9), wraplength=180)
        ...
        self.metrics_labels = {}
        metrics = ttk.LabelFrame(content, text="Key Metrics", padding=10)
        metrics.pack(fill=tk.X, pady=20)
        ...
        for lbl, key in [
            ...
            self.metrics_labels[key] = ttk.Label(f, text="N/A", font=('Arial', 12))
            self.metrics_labels[key].pack()

        viz = ttk.LabelFrame(content, text="Visualization", padding=10)
        viz.pack(fill=tk.BOTH, expand=True)

        self.fig = Figure(figsize=(12, 8), dpi=100)
        self.canvas = FigureCanvasTkAgg(self.fig, viz)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        self.toolbar = NavigationToolbar2Tk(self.canvas, viz)
        self.toolbar.update()

        for ev in ('<MouseWheel>', '<Button-4>', '<Button-5>'):
            self.canvas.get_tk_widget().bind(ev, self.on_mousewheel)

        self.enable_pan()

        self.info_labels = {}
        info = ttk.LabelFrame(content, text="Data Information", padding=10)
        info.pack(fill=tk.X, pady=20)
        ...
        for lbl, key in [
            ...
            f.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=10)
            ttk.Label(f, text=lbl, font=('Arial', 10, 'bold')).pack()
            self.info_labels[key] = ttk.Label(f, text="N/A", font=('Arial', 9))
    ...
    def _make_section(self, parent, title, btns):
        ...
        for txt, cmd in btns:
            ttk.Button(f, text=txt, command=cmd, width=18).pack(pady=2)

    def _make_combo(self, parent, title, values, cmd, default=None):
        f = ttk.LabelFrame(parent, text=title, padding=10)
        f.pack(fill=tk.X, padx=10, pady=10)
        var = tk.StringVar(value=default or "")
        combo = ttk.Combobox(f, textvariable=var, values=values, state="readonly", width=18)
        combo.pack()
    ...
    def _after_load(self, filename):
        ...
        self.update_metrics()
        self.update_info()
        self.viz_var.set("Sum of Sales by Segment")
        self.update_visualization()
    ...
    def _zoom(self, factor):
        ...
        ax.set_xlim(x0 + xr, x1 - xr)
        ax.set_ylim(y0 + yr, y1 - yr)
        self.canvas.draw()
    ...
    def _on_motion(self, event):
        if self._press_event is None or event.inaxes != self._press_event.inaxes:
            return
        ax = event.inaxes
    ...
    def update_info(self):
        d = self.data
        self.info_labels['shape'].config(text=str(d.shape))
    ...
    def viz_sum_of_sales_by_segment(self, ax):
        ...
        ax.set_title('Sum of Sales by Segment')
    ...
    def viz_count_of_order_id_by_customer_name(self, ax):
        ...
        ax.set_title('Count of Order ID by Customer Name (Top 15)')
    ...
    def viz_sum_of_sales_by_customer_name(self, ax):
        ...
        ax.set_xticklabels(cs.index, rotation=45, ha='right')
        ax.set_title('Sum of Sales by Customer Name (Top 20)')
        ax.set_ylabel('Sales (₹)')
        ax.grid(True, axis='y', linestyle='--', alpha=0.7)
    ...
    def viz_sum_of_sales_by_month_and_region(self, ax):
        ...
        ax.set_title('Sum of Sales by Month and Region')
    ...
    def viz_top_5_products_by_revenue(self, ax):
        ...
        ax.set_xticklabels(pr.index, rotation=45, ha='right')
    ...
    def viz_profitability_by_sub_category(self, ax):
        ...
        ax.set_xticklabels(sc.index, rotation=45, ha='right')
